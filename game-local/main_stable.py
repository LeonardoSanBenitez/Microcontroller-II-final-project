# This is mostly based on commit 329a3efa4011f96dde7e977a4a1e20d943f6f469
import numpy as np
...
class EnvBlob(Env):
    ...
    def reset(self):
        ...
        inits = [0]*6
        inits[0] = np.random.choice(SIZE)#np.random.default_rng().choice(SIZE, size=6, replace=False)
        inits[1] = np.random.choice(SIZE)
        while True:
            inits[2] = np.random.choice(SIZE)
            inits[3] = np.random.choice(SIZE)
            if inits[2]!=inits[0] or inits[3]!=inits[1]:
                break
        while True:
            inits[4] = np.random.choice(SIZE)
            inits[5] = np.random.choice(SIZE)
...
episode_rewards = []
AI = IntelligentAgent(start_q_table=start_q_table, epsilon=EPSILON)
env = EnvBlob()
